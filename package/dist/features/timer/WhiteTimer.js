...
var WhiteTimer = function WhiteTimer() {
  ...
  var timer = (0, _reactTimerHook.useTimer)({
    ...
    onExpire: function onExpire() {
      dispatch(mode.timeOver({ color: _Pgn2.default.symbol.WHITE }));
  ...
  (0, _react.useEffect)(function () {
    if (isInitialMount.name) {
      ...
      if (state.board.isMate || state.mode.play.draw || state.mode.play.resign || state.mode.play.leave || state.mode.play.timer.over) {
        timer.pause();
      } else if (state.board.turn === _Pgn2.default.symbol.WHITE) {
        timer.resume();
      } else {
        var now = new Date();
  ...
  return _react2.default.createElement(
    _material.Box,
    { component: 'span', style: { marginRight: 10 } },
    timer.minutes,
    ':',
    timer.seconds
  );
